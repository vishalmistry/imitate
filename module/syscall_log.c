...
syscall_log_entry_t *get_next_syscall_log_entry(unsigned short call_no)
{
    ...
    {
        ERROR("Process %d (PID: %d) is expected to replay system call %d, but is actually trying to replay %d!", process->child_id, current->pid, log_entry->call_no, call_no);
...
void seek_to_next_syscall_entry(void)
{
    ...
    if (next_offset >= monitor->syscall_size)
    {
        ...
        {
            ERROR("System call log for PID %d is empty. Killing process.", current->pid);
